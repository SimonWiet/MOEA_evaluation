...
def simulate(M, BLOCK_SIZE, cxpb, k=2, benchmark="OMM", verb=False, NGEN=10000):
    ...
    if benchmark == "OMM":
        ...
        def evaluate(ind):
            half = int(n/2)
            first_half = np.sum(ind[:half])
            ones = onesPerBlock(ind[half:])
            vals = first_half + np.concatenate((ones, BLOCK_SIZE - ones))
            return (vals, first_half == n/2)
    ...
    return semo(compare, evaluate, BLOCK_SIZE, M, n, SIZE_PARETO_FRONT, NGEN, cxpb, verb)
    
    

def mutate(ind):
    idx = np.random.choice(ind.shape[0])
    offspring = ind.copy()
    offspring[idx] = 1-ind[idx]
    return offspring
...
def mutateAndCrossover(pop, cxpb):
...
def semo(compare, evaluate, BLOCK_SIZE, M, n, SIZE_PARETO_FRONT, NGEN, cxpb, verb):
    ...
    for gen in range(1, NGEN):
        offspring = mutateAndCrossover(pop, cxpb)
...
if __name__== "__main__":
    ...
    if benchmark == 'OJZJ':
        raise Exception('semo.py: SEMO is not able to solve OJZJ.'.format(benchmark))
